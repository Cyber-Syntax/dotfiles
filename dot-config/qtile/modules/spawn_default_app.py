...
def spawn_default_app(
    qtile,
    groups: list,
    default_apps: list[str],
    use_run_as_unset_default: bool = False,
    unset_default_app: str = None,
) -> None:
    ...
    group_index = (
        get_index(qtile.current_group.name) if qtile.current_group is not None else None
    )
    if (
        qtile.current_group is not None
        and group_index is not None
        and default_apps is not None
        and default_apps[group_index] is not None
    ):
